...
import gdv.ucm.appnonogram.BUTTONS.ButtonNext;
import gdv.ucm.appnonogram.BUTTONS.ButtonShare;
...
public class WinScene implements StateA {
    ...
    public void update(double deltaTime) {
        if(this.engine.getContext().getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            this.bBack.setPos(50,this.gr.getHeightLogic()/4);
            this.bShare.setPos(50,(this.gr.getHeightLogic()/4)*2);
            if(!random)
                this.bNext.setPos(50,(this.gr.getHeightLogic()/4)*3);
        }
        else
        {
