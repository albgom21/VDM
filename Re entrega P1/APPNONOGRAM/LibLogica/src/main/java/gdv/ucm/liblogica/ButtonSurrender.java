...
public class ButtonSurrender implements IInterface {
    ...
    public boolean handleEvent(IInput.Event e) {
        ...
        if(e.type == IInput.InputTouchType.PRESSED && //click
          ...
          && mY >= this.gr.logicToRealY(y) - (this.gr.scaleToReal(h)/2) + this.gr.getBorderTop() && mY <= this.gr.scaleToReal(h) + this.gr.logicToRealY(y) - (this.gr.scaleToReal(h)/2) + this.gr.getBorderTop())){ // dentro del cuadrado
