...
public class Board implements IInterface {
   ...
   public Board(int w, int h, int borderTop) {
      width = w;
      height = h;
      int cont = 0;
      ...
      for (int i = 0; i < width; ++i) {
         for (int j = 0; j < height; ++j) {
            ...
            if(valorEntero==0)
               board[i][j] = new Cell(i,j,width, height,false, CellState.GRAY, borderTop);
            else{
               board[i][j] = new Cell(i,j,width, height,true, CellState.GRAY, borderTop);
               cont++;
            }
      ...
      if(cont == 0 || cont == w*h){ //Por si no hay ninguna celda sol, o son todas
         ...
         board[i][j] = new Cell(i,j,width, height,cont == 0, CellState.GRAY, borderTop);
      }
   }
