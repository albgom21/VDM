...
public class ButtonRetry implements InterfaceA {
    ...
    ButtonRetry(String filename, EngineA engine, int x, int y, int w, int h, String file, int cols, int fils){
        this.engine = engine;
        this.file = file;
        this.cols = cols;
        this.fils = fils;
    ...
    public boolean handleEvent(InputA.Event e) {
        ...
        if(e.type == InputA.InputTouchType.NORMAL_PRESSED && //click
            ...
            if(this.cols != 0)
            {
                ...
                MainSceneRead scene = new MainSceneRead(this.engine,this.file);
                engine.setCurrentScene(scene);
            }
